    ...
    try:
        if prev_points is not None and prev_points.size > 0:
            next_points, status, _ = cv2.calcOpticalFlowPyrLK(
                prev_frame, gray_frame, prev_points, None, **lk_params
            )
            for new, old in zip(next_points, prev_points):
                ...
                cv2.putText(
                    frame,
                    f'Speed: {speed:.2f} Direction: {direction:.2f}',
                    (int(a), int(b)),
...
def detect_and_alert(frame):
    ...
    if frame_count % 1 == 0:
        ...
        detections_summary["total_detections"] = (
            detections_summary["pedestrians_detected"] +
...
def upload_video_feed():
    ...
    if file:
        ...
        file.save(os.path.join(app.config['UPLOAD_FOLDER'], filename))
        return Response(
            detect_file(os.path.join(app.config['UPLOAD_FOLDER'], filename)),
